...
async function submit() {
    ...
    try {
        await addServer({ 
            ...
            store, 
            submissionState,
            mustMatchId: props.server.server.id
        })
...
<template>
    <ModalWindow
        class="update-server-modal"
        :title="'Update Server [' + server.server.hostname + ']'"
        :buttons="[ 'close' ]"
        @onButtonTap="handleButtonTap"
    >
        <template v-slot:content>
            <div class="instructions">
                Enter the new location of <b>{{ server.server.hostname}}</b>:
            </div>
            <div class="errors">
                <div 
